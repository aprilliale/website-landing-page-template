...
export default function App() {
  ...
  function fadeIn(video: HTMLVideoElement) {
    ...
    const fromOpacity = video.style.opacity !== '' ? parseFloat(video.style.opacity) : 0;
  ...
  function fadeOut(video: HTMLVideoElement, onDone: () => void) {
    cancelRaf();
    const start = performance.now();
    const fromOpacity = video.style.opacity !== '' ? parseFloat(video.style.opacity) : 1;
  ...
  useEffect(() => {
    ...
    function handleTimeUpdate() {
      ...
      if (!fadingOutRef.current && remaining <= FADE_OUT_THRESHOLD) {
